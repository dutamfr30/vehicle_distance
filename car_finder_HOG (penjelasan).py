...
from settings import CALIBRATION_FILE_NAME, PERSPECTIVE_FILE_NAME, UNWARPED_SIZE, ORIGINAL_SIZE
...
class Car:
    # menginisiasi objek car dengan beberapa parameter
    ...
    # menghitung posisi relatif mobil terhadap kendaraan
    ...
    # mengembalikan bounding box hasil filter mobil
    def get_window(self):
        return self.filtered_bbox.output()
    
    # ketika menemukan kendaraan dalam frame, maka diatur frame yang hilang menjadi 0 
    # mengupdate status tampilan kendaraan jika jumlah frame melebihi ambang batas
    def one_found(self):
        self.num_lost = 0
        if not self.display:
            self.num_found += 1
    ...
    # ketika tidak menemukan kendaraan dalam frame, maka akan mengubah status ditemukan menjadi false
    def one_lost(self):
        self.num_found = 0
        self.num_lost += 1
        if self.num_lost > 5:
    ...
    # memperbarui informasi kendaraan berdasarkan bounding box baru, apakah terdapat tumpang tindih
    ...
    # menggambar bounding box dan posisi mobil
    def draw(self, img, color=(255, 0, 0), thickness=2):
        if self.display:
            window = self.filtered_bbox.output().astype(np.uint32)
            cv.rectangle(img, (window[0], window[1]), (window[2], window[3]), color, thickness)
            if self.has_position:
                cv.putText(img, "RPos : {:6.2f}m".format(self.position.output()[0]), (int(window[0]), int(window[1]-5)),
                           cv.FONT_HERSHEY_PLAIN, fontScale=1.25, thickness=3, color=(255, 255, 255))    
                cv.putText(img, "RPos : {:6.2f}m".format(self.position.output()[0]), (int(window[0]), int(window[1]-5)),
                           cv.FONT_HERSHEY_PLAIN, fontScale=1.25, thickness=2, color=(0, 0, 0))
                cv.putText(img, "RVel: {:6.2f}km/h".format(self.position.speed()[0]*self.fps*3.6), (int(window[0]), int(window[3]+20)),
                           cv.FONT_HERSHEY_PLAIN, fontScale=1.25, thickness=3, color=(255, 255, 255))    
                cv.putText(img, "RVel: {:6.2f}km/h".format(self.position.speed()[0]*self.fps*3.6), (int(window[0]), int(window[3]+20)),
                           cv.FONT_HERSHEY_PLAIN, fontScale=1.25, thickness=2, color=(0, 0, 0))   
...
class CarFinder:
    def __init__(self, size, hist_bins, small_size, orientations=12, pix_per_cell=8, cell_per_block=2,
                 ...
                 warped_size=None, transform_matrix=None, pixel_per_meter=None):
        self.size = size
    ...
    # menghitung dan mengembalikan jumlah fitur yang dihasilkan
    ...
    def get_features(self, img):
        img_resize = cv.resize(img, (self.size, self.size), interpolation=cv.INTER_LINEAR)
        img_resize = (np.sqrt(img_resize.astype(np.float32)/255)*255).astype(np.uint8) # mengubah tipe citra menjadi float32 dan melakukan normalisasi pikselnya
        img_LUV = cv.cvtColor(img_resize, cv.COLOR_RGB2LUV) # mengubah citra menjadi LUV
        img_feature = cv.resize(img_LUV, (self.small_size, self.small_size), interpolation=cv.INTER_LINEAR)
        hist_l = np.histogram(img_LUV[:, :, 0], bins=self.hist_bins, range=self.hist_range) # menghitung histogram L (Lightness)
        width = 0.7 * (hist_l[1][1] - hist_l[1][0])
        center = (hist_l[1][:-1] + hist_l[1][1:]) / 2
        hist_u = np.histogram(img_LUV[:, :, 1], bins=self.hist_bins, range=self.hist_range) # menghitung histogram U (Green-Red)
        hist_v = np.histogram(img_LUV[:, :, 2], bins=self.hist_bins, range=self.hist_range) # menghitung histogram V (Blue-Yellow)
        ...
        features_v = hog(img_LUV[:, :, 2], self.orientations, pixels_per_cell=(self.pix_per_cell, self.pix_per_cell),
                         cells_per_block=(self.cell_per_block, self.cell_per_block), transform_sqrt=False,
                         feature_vector=True)
        return np.hstack((img_feature.ravel(), hist_l[0], hist_u[0], hist_v[0], features_l, features_u, features_v)) # menggabungkan semua fitur
    
    def car_find_roi(self, img, size, roi, overlap):
        assert self.scaler is not None, "CarFinder error -> Scaller has to be initialized"
        ...
        img_roi = cv.resize(img_roi, (new_width, new_height))
        img_roi = (np.sqrt(img_roi.astype(np.float32)/255)*255).astype(np.uint8)
        img_roi = cv.cvtColor(img_roi, cv.COLOR_RGB2LUV)
        img_small = cv.resize(img_roi, (math.ceil(new_width*self.small_size/float(self.size)), 
        ...
        for col in range(n_horizontal):
            for row in range(n_vertical):
                img_ = img_roi[row*shift_roi:row*shift_roi+self.size, col*shift_roi:shift_roi+self.size]
    ...
    def find_cars(self, img, threshold = 1, reset = False):
        heatmap = np.zeros_like(img[:, :, 1]) # membuat variabel heatmap dengan ukuran yang sama dengan img
        car_windows = [] # list untuk menyimpan jendela yang berpotensi terdapat mobil
        if reset:
            self.cars = []
            self.first = True
        for size, roi in zip(self.window_sizes, self.window_rois):
            car_windows += self.car_find_roi(img, size, roi, overlap=0.75)
        # mengupdate heatmap dan menambahkan 1 pada setiap piksel di area yang terdapat mobil
        for car in self.cars:
            window = car.get_window()
            heatmap[int(window[1]):int(window[3]), int(window[0]):int(window[2])] += 1

        for window in car_windows:
            heatmap[int(window[1]):int(window[3]), int(window[0]):int(window[2])] += 1

        heatmap = heatmap > threshold # mengubah nilai heatmap menjadi boolean
        label_img, labels = label(heatmap) # mengelompokkan piksel yang berdekatan menjadi region terpisah
        bboxes = [] # list berisi koordinat bounding box
        for lbl in range(labels):
            ...
            near = cv.inRange(car_img, medi - np.array([35, 35, 35]), medi + np.array([35, 35, 35]))
            if near is not None:
        ...
        # membuat objek mobil baru dan ditambahkan ke daftar mobil
        for bbox in bboxes:
            self.cars.append(Car(bbox, self.first, self.warped_size, self.transformation_matrix, self.pixel_per_meter))

        # membuat daftar mobil sementara 
        tmp_cars = []
        for car in self.cars:
            if car.found:
                tmp_cars.append(car)
        self.cars = tmp_cars # memperbarui daftar mobil dengan daftar mobil sementara
        self.first = False # mengubah nilai first menjadi False karna bukan frame pertama lagi

    # menggambar bounding box pada citra
    def draw_cars(self, img):
        i2 = np.copy(img) # membuat copy dari img
        for car in self.cars:  
            car.draw(i2) # menggambar bounding box pada citra salinan
        return i2 # mengembalikan citra salinan yang sudah digambar bounding box
