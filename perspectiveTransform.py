...
for img_path in straight_images:
    img = pimg.imread(img_path)
    img = cv.undistort(img, cam_matrix, dist_coeffs)
    img = cv.warpPerspective(img, M, settings.UNWARPED_SIZE)
    img_hls = cv.cvtColor(img, cv.COLOR_RGB2HLS)
    mask = img_hls[:, :, 1] > 128 
    mask[:, :50] = 0 
    mask[:, -50:] = 0 
    mom = cv.moments(mask[:, :settings.UNWARPED_SIZE[0]//2].astype(np.uint8)) 
    x1 = mom["m10"]/mom["m00"] 
    mom = cv.moments(mask[:, settings.UNWARPED_SIZE[0]//2:].astype(np.uint8))
    x2 = settings.UNWARPED_SIZE[0]//2 + mom["m10"]/mom["m00"]
    cv.line(img, (int(x1), 0), (int(x1), settings.UNWARPED_SIZE[1]), (255, 0, 0), 3)
    cv.line(img, (int(x2), 0), (int(x2), settings.UNWARPED_SIZE[1]), (0, 0, 255), 3)
    if (x2-x1<min_wid):
        min_wid = x2-x1
